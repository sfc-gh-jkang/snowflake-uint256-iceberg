...
SELECT token_addr,
       COALESCE(NULLIF(LTRIM(TO_VARCHAR(FLOOR(t0+k1))||LPAD(TO_VARCHAR(FLOOR(r1)),20,'0')
       ||LPAD(TO_VARCHAR(FLOOR(r2)),20,'0')||LPAD(TO_VARCHAR(FLOOR(r3)),20,'0'),'0'),''),'0') AS exact_total
FROM c3;
...
c3 AS (SELECT g,t0,r3,r2, MOD(t1+k2,100000000000000000000) r1,
              ((t1+k2)-MOD(t1+k2,100000000000000000000))/100000000000000000000 k1 FROM c2),
s  AS (SELECT g, COALESCE(NULLIF(LTRIM(TO_VARCHAR(FLOOR(t0+k1))||LPAD(TO_VARCHAR(FLOOR(r1)),20,'0')
       ||LPAD(TO_VARCHAR(FLOOR(r2)),20,'0')||LPAD(TO_VARCHAR(FLOOR(r3)),20,'0'),'0'),''),'0') AS exact_total FROM c3),
